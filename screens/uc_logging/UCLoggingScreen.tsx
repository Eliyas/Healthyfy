...
import { CreateReportInput, ReportType, SearchableSortDirection, SearchableTagSortableFields } from "../../src/API";
...
export default function UCLoggingScreen() {
  ...
  const handleSlideEnd = () => {
    ...
    setCurrentPage(carouselRef.current.getCurrentIndex());
  }
  ...
  const saveCurrentTag = async () => {
    ...
    setIsLoading(true);
    try {
      const newTag = await client.graphql({
  ...
  const deleteTag = async (tagId) => {
    ...
    try {
      const removedTag = await client.graphql({
